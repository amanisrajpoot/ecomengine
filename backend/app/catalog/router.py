...
@router.patch(
    "/businesses/{business_id}/categories/{category_id}",
    response_model=CategoryRead,
)
...
async def list_addons(
    ...
    _ = ctx
    tid = _require_tenant(tenant_id)
    rows = await service.list_addons(
        db, tenant_id=tid, business_id=business_id, active_only=active_only
    )
...
@router.patch(
    "/businesses/{business_id}/addons/{addon_id}",
    response_model=AddonRead,
)
...
@router.post(
    "/businesses/{business_id}/products/{product_id}/addon-links",
...
async def link_product_addon(
    ...
    _ = ctx
    tid = _require_tenant(tenant_id)
    link = await service.link_product_addon(
        db,
        tenant_id=tid,
        business_id=business_id,
        product_id=product_id,
    ...
    return ProductAddonLinkRead.model_validate(link)
...
async def list_product_addon_links(
    ...
    _ = ctx
    tid = _require_tenant(tenant_id)
    rows = await service.list_product_addon_links(
        db, tenant_id=tid, business_id=business_id, product_id=product_id
    )
...
@router.post(
    "/businesses/{business_id}/bundles",
...
async def list_bundles(
    ...
    _ = ctx
    tid = _require_tenant(tenant_id)
    rows = await service.list_bundles(
        db, tenant_id=tid, business_id=business_id, active_only=active_only
    )
...
@router.patch(
    "/businesses/{business_id}/bundles/{bundle_id}",
    response_model=BundleRead,
)
